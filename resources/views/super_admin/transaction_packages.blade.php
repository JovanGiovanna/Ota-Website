            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Addons</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            @if($transaction->packages->count() > 0)
                                @foreach($transaction->packages as $package)
                                    <div class="text-sm text-gray-900">{{ $package->name_package }}</div>
                                    <div class="text-sm text-gray-500">{{ \Illuminate\Support\Str::limit($package->description, 25) }}</div>
                                @endforeach
                            @else
                                <div class="text-sm text-gray-500">No packages</div>
                            @endif
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            @if($transaction->addons->count() > 0)
                                @foreach($transaction->addons as $addon)
                                    <div class="text-sm text-gray-900">{{ $addon->addons }}</div>
                                    <div class="text-sm text-gray-500">Qty: {{ $addon->pivot->quantity ?? 1 }} - Rp {{ number_format($addon->price, 0, ',', '.') }}</div>
                                @endforeach
                            @else
                                <div class="text-sm text-gray-500">No addons</div>
                            @endif
                        </td>
