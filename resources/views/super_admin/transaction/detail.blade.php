...
<div class="container mx-auto px-6 py-8">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="#" class="flex items-center text-blue-600 hover:text-blue-800">Kembali</a>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="space-y-6">
            ...
            <div class="bg-yellow-50 border border-yellow-300 p-6 rounded-lg shadow-md">
                ...
                <form action="{{ route('super_admin.transaction.approve', $booking) }}" method="POST" class="mb-3">@csrf<button type="submit" class="w-full px-4 py-3 bg-green-600 text-white font-semibold rounded-lg">✅ Setujui Booking</button></form>
                <form action="{{ route('super_admin.transaction.reject', $booking) }}" method="POST">@csrf<button type="submit" class="w-full px-4 py-3 bg-red-600 text-white font-semibold rounded-lg">❌ Tolak/Batalkan Booking</button></form>
            </div>
            @endif
            ...
            <div class="bg-white p-6 rounded-lg shadow-md">
                ...
                <form action="{{ route('super_admin.transaction.verify_payment', $booking) }}" method="POST">@csrf<button type="submit" class="w-full px-4 py-3 bg-blue-600 text-white font-semibold rounded-lg">✔️ Verifikasi Pembayaran (Tandai Completed)</button></form>
            </div>
            @endif
            ...
            <div class="bg-white p-6 rounded-lg shadow-md">
                ...
                <form action="{{ route('super_admin.transaction.process_refund', $booking) }}" method="POST">@csrf<button type="submit" class="w-full px-4 py-3 bg-red-600 text-white font-semibold rounded-lg">💸 Proses Pengembalian Dana</button></form>
            </div>
            @endif
