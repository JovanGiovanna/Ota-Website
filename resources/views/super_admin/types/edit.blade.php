...
<div class="px-4 py-6 sm:px-0">
    <div class="max-w-md mx-auto bg-white shadow overflow-hidden sm:rounded-md">
        <div class="px-4 py-5 sm:px-6">
            <h3 class="text-lg leading-6 font-medium text-gray-900">Edit Type</h3>
            <p class="mt-1 max-w-2xl text-sm text-gray-500">Update type information</p>
        </div>

        <form method="POST" action="{{ route('super_admin.types.update', $type) }}" class="px-4 py-5 sm:px-6 space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                <label class="flex items-center">
                    <input type="checkbox" name="status" value="1" {{ old('status', $type->status) ? 'checked' : '' }}
