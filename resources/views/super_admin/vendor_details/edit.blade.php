        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <form action="{{ route('super_admin.vendor_details.update', $vendorInfo->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" id="description" rows="4" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" required>{{ old('description', $vendorInfo->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="phone" id="phone" value="{{ old('phone', $vendorInfo->phone) }}" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                    @error('phone')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    <label class="flex items-center">
                        ...
                        <input type="checkbox" name="is_verified" value="1" {{ old('is_verified', $vendorInfo->is_verified) ? 'checked' : '' }} class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                        <span class="ml-2 text-sm font-medium text-gray-700">Verified Vendor</span>
                    </label>
                    @error('is_verified')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
