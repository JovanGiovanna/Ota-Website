...
<div class="px-4 py-6 sm:px-0">
    <div class="max-w-2xl mx-auto bg-white shadow overflow-hidden sm:rounded-md">
        <div class="px-4 py-5 sm:px-6">
            ...
            <p class="mt-1 max-w-2xl text-sm text-gray-500">Update package information</p>
        </div>

        <form method="POST" action="{{ route('super_admin.packages.update', $package) }}" enctype="multipart/form-data" class="px-4 py-5 sm:px-6 space-y-6">
            @csrf
            @method('PUT')

            <div>
                <label for="name_package" class="block text-sm font-medium text-gray-700">Package Name</label>
                <input type="text" name="name_package" id="name_package" value="{{ old('name_package', $package->name_package) }}" required
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 @error('description') border-red-500 @enderror">{{ old('description', $package->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                @if($package->image)
                    @php $currentImages = json_decode($package->image, true); @endphp
                    @if(is_array($currentImages) && count($currentImages) > 0)
                        <div class="mb-2 grid grid-cols-3 gap-2">
                            @foreach($currentImages as $index => $image)
                                <div class="relative">
                                    <img src="{{ asset('storage/' . $image) }}" alt="Current Image {{ $index + 1 }}" class="w-32 h-32 object-cover rounded-md">
                    ...
                    @endif
                @endif
                <input type="file" name="images[]" id="images" accept="image/*" multiple class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 @error('images') border-red-500 @enderror">
                <p class="mt-1 text-sm text-gray-500">Select multiple images to add. Leave empty to keep current images.</p>
                @error('images')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
                @error('images.*')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" name="price_publish" id="price_publish" value="{{ old('price_publish', $package->price_publish) }}" step="0.01" min="0" required
